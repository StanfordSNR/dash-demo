...
DASH_FETCH_CHUNK_RE = re.compile(r'getNextFragment - request is http://[\w:\d]+/static/dash/media/(\d+x\d+-\d+)/(\d+).m4s')
...
def main(logfile):
    with open(logfile) as fp:
        for line in fp:
            ...
            dash_fetch_match = DASH_FETCH_CHUNK_RE.search(line)
            if dash_fetch_match:
                print(timestamp, 'dashfetch', dash_fetch_match.group(1),
                      dash_fetch_match.group(2))
